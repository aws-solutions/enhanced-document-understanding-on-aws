...
for jarFile in `find . -mindepth 1 -maxdepth 1 -iname '*.jar'`; do
    pfname="$(basename -- $jarFile .jar)"
    fname="$(echo $pfname | sed -e 's/\.//g')"
    mv $jarFile $fname.jar

    cp $fname.jar $build_dist_dir
    rm $fname.jar
done

publish_java_dependency_report() {
    lambda_function=$1
    cd $lambda_function

    echo "----------Checking $lambda_function for Java dependencies-------------------"
...
cd $source_dir/lambda
publish_java_dependency_report layers/custom-java-sdk-config
publish_java_dependency_report redact-content
