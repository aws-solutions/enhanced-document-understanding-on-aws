...
import io
import zipfile
from test.fixtures.copy_sample_documents import lambda_event, setup_samples
...
def test_evn_setup_with_resource_props_wrong_value(monkeypatch, lambda_event):
    with pytest.raises(ValueError):
        monkeypatch.setitem(lambda_event, RESOURCE_PROPERTIES, value={RESOURCE: "NOT_SAMPLES"})
...
def verify_bucket_contents(s3, bucket_name, prefix):
    ...
    with pytest.raises(botocore.exceptions.ClientError):
        s3.Object(bucket_name, f"{prefix}/non_existing.sample-documents.zip").load()


def test_setup_samples_fixture(setup_samples):
    lambda_event, s3 = setup_samples
...
@pytest.mark.parametrize("requestType", ["Create", "Update", "Delete"])
def test_execute_call_success(setup_samples, mock_lambda_context, requestType):
    lambda_event, s3_resource = setup_samples
    lambda_event["RequestType"] = requestType
...
@pytest.mark.parametrize("requestType", ["Create", "Update", "Delete"])
def test_execute_call_success_with_generated_physical_resource_id(setup_samples, mock_lambda_context, requestType):
    lambda_event, s3_resource = setup_samples
    lambda_event["RequestType"] = requestType
...
def test_execute_call_with_bad_archive(tmp_path, setup_samples, mock_lambda_context, requestType):
    lambda_event, s3_resource = setup_samples
    lambda_event["RequestType"] = requestType
...
@pytest.mark.parametrize("requestType", ["Create", "Update"])
def test_execute_call_with_wrong_source_bucket(monkeypatch, setup_samples, mock_lambda_context, requestType):
    lambda_event, _ = setup_samples
    lambda_event["RequestType"] = requestType
...
@pytest.mark.parametrize("requestType", ["Create", "Update", "Delete"])
def test_execute_call_with_wrong_destination_bucket(monkeypatch, setup_samples, mock_lambda_context, requestType):
    lambda_event, _ = setup_samples
    lambda_event["RequestType"] = requestType
...
@pytest.mark.parametrize("requestType", ["Create", "Update", "Delete"])
def test_lambda_handler(setup_samples, mock_lambda_context, requestType):
    lambda_event, s3_resource = setup_samples
    lambda_event["RequestType"] = requestType
