...
from test.fixtures.copy_web_ui_events import web_ui_copy_setup, SAMPLE_JSON_VALUE
...
@pytest.mark.parametrize(
    "s3, setup_email_templates, mock_lambda_context, patch_powertools",
    [
        ("s3", "setup_email_templates", "mock_lambda_context", "patch_powertools"),
    ],
    indirect=["s3", "setup_email_templates", "mock_lambda_context", "patch_powertools"],
)
@pytest.mark.parametrize("requestType", ["Create", "Update", "Delete"])
@mock.patch("cfn_response.http")
def test_handler_for_email_template_copy_success(
    s3, setup_email_templates, mock_lambda_context, patch_powertools, requestType
):
    event_obj, s3 = setup_email_templates
...
@mock.patch("cfn_response.http")
def test_handler_for_email_template_copy_error(
    s3, setup_email_templates, mock_lambda_context, patch_powertools, requestType
):
    event_obj, s3 = setup_email_templates
    ...
    mpatch.setitem(event_obj[RESOURCE_PROPERTIES], RESOURCE, "NOT_COPY_TEMPLATE")
