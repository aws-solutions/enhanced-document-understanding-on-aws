...
def verify_env_setup(event):
    ...
    """
    if event[RESOURCE_PROPERTIES][RESOURCE] != operation_types.COPY_SAMPLE_DOCUMENTS:
...
def delete(s3_resource, destination_bucket_name, destination_prefix):
    ...
    requires delete permissions on the destination bucket. All objects including any versions of the sample documents will be deleted from the destination
    bucket.

    Args:
        s3_resource (boto3.s3): A boto3 resource for the S3 service
        destination_bucket_name (str): Bucket name created during deployment where the sample documents will be uploaded after unzipping them from the archive
        destination_prefix (str): The key prefix under which all the sample documents exist

    Raises
        botocore.exceptions.ClientError: Failures related to S3 bucket operations
    """
    logger.warning(
        f"Deleting all sample documents from {destination_bucket_name} including any versions of those objects"
    )
    try:
...
def create(
    ...
    """This method implements the operations to be performed when a `create` event is received by a AWS CloudFormation
    custom resource. As implementation, this method copies the sample documents from the source bucket to the destination bucket.
    ...
    Args:
        ...
        source_bucket_name (str): Bucket name which contains the asset archive with sample documents
        source_prefix (str): The prefix under the source bucket which corresponds to the archive for sample documents
        destination_bucket_name (str): Bucket name created during deployment where sample documents will be uploaded after unzipping them from the archive
        destination_prefix (str): The name of the folder (path), under the destination bucket where the sample documents should be copied to
    ...
    for filename in zip_archive.namelist():
        logger.info(f"Copying {filename} sample documents to {destination_bucket_name}")
        try:
...
def execute(event, context):
    """This method copies sample documents to the Bucket created by CloudFormation when deploying the application. CloudFormation fires 3 events,
    'Create', 'Update', and 'Delete'. For 'Create' event, this method calls :meth:`create`, and :meth:`delete` for 'Delete' event.
