...
/**
 * This class provides the functionality to fetch comprehend medical inference from S3 and transforms it to be persisted
 * to the OpenSearch serverless collection.
 */
class MedicalEntityStrategy {
    /**
    ...
    /**
     * Fetch comprehend medical inference from S3 and transforms it.
     */
        ...
        for (const document of this.documents) {
            ...
            try {
                const inference = await SharedLib.getInferenceFromS3(
                    this.caseId,
                    documentId,
                    SharedLib.InferenceTypes.MEDICAL_ENTITY,
                    this.accountId,
                    s3Client
                );
    ...
    transformInference(inference) {
        ...
        for (const page of inference) {
            for (const entity of page.Entities) {
                if (!transformedInference.hasOwnProperty(entity.Category)) {
                    transformedInference[entity.Category] = [];
                }
                transformedInference[entity.Category].push(entity.Text);
            }
        }
...
module.exports = { MedicalEntityStrategy };
