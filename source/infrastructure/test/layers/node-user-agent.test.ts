    ...
    it('should throw an error if the runtime is python', () => {
        try {
            ...
            expect((error as Error).message).toEqual(
                `This lambda function uses a runtime that is incompatible with this layer (${COMMERCIAL_REGION_LAMBDA_PYTHON_RUNTIME.name} is not in [nodejs18.x])`
            );
        }
    });
            ...
            new lambda.Function(stack, 'TestFunction', {
                ...
                layers: [
                    new NodeUserAgentLayer(stack, 'AWSUserAgentConfigLayer', {
                        entry: '../lambda/layers/aws-node-user-agent-config',
                        description: 'This layer configures AWS Node SDK initialization to send user agent information',
                        compatibleRuntimes: [COMMERCIAL_REGION_LAMBDA_NODE_RUNTIME]
                    })
                ]
            });
        } catch (error) {
            expect((error as Error).message).toEqual(
                `This lambda function uses a runtime that is incompatible with this layer (${lambda.Runtime.NODEJS_18_X.name} is not in [${COMMERCIAL_REGION_LAMBDA_NODE_RUNTIME.name}])`
            );
        }
    });
});
