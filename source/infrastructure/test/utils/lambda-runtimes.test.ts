...
describe('When add lambda runtime command', () => {
    ...
    describe('for a java function with govcloud region same as commercial region', () => {
        let lambdaRuntimeCommandFactory: LambdaRuntimeCommandFactory;
        beforeAll(() => {
            jest.doMock('../../lib/utils/constants', () => ({
                COMMERCIAL_REGION_LAMBDA_JAVA_RUNTIME: lambda.Runtime.JAVA_11,
                GOV_CLOUD_REGION_LAMBDA_JAVA_RUNTIME: lambda.Runtime.JAVA_11
            }));
        ...
        it('should return a Token for java', () => {
            lambdaRuntimeCommandFactory.getRuntimeCommand(lambda.RuntimeFamily.JAVA);
            const result = lambdaRuntimeCommandFactory.getRuntimeCommand(lambda.RuntimeFamily.JAVA).getLambdaRuntime();
            expect(result).toContain('java11');
        });
    });

    describe('for a java function with runtime in the govcloud lower than as commercial region', () => {
        let lambdaRuntimeCommandFactory: LambdaRuntimeCommandFactory;
        beforeAll(() => {
            lambdaRuntimeCommandFactory = new LambdaRuntimeCommandFactory(
                new cdk.CfnCondition(new cdk.Stack(), 'isGovCloudPartition', {
